...
import static org.firstinspires.ftc.teamcode.arm.PidTuner.ZNMode.ZNModeBasicPID;
import static org.firstinspires.ftc.teamcode.arm.PidTuner.ZNMode.ZNModeLessOvershoot;

/**
 * https://github.com/jackw01/arduino-pid-autotuner
 */
public class PidTuner {
    public static enum ZNMode {
        ZNModeBasicPID,
        ZNModeLessOvershoot,
        ZNModeNoOvershoot
    }

    private float targetInputValue = 0;
    private float loopInterval = 0;
    private float minOutput, maxOutput;
    private ZNMode znMode = ZNMode.ZNModeNoOvershoot;
    private int cycles = 10;

    private int i;
    private boolean output;
    private float outputValue;
    private long microseconds, t1, t2, tHigh, tLow;
    private float max, min;
    private float pAverage, iAverage, dAverage;
    ...
    public void startTuningLoop(long us) {
        ...
        max = -1000000000000f; // Max input
        min = 1000000000000f; // Min input
    ...
    public float tunePID(float input, long us) {
        ...
        if (!output && input < targetInputValue) {
            ...
            float ku = (float) ((4.0 * ((maxOutput - minOutput) / 2.0)) / (Math.PI * (max - min) / 2.0));
